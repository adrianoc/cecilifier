...
namespace Cecilifier.Core.Naming
{
    public class DefaultNameStrategy : INameStrategy
    {
        ...
        public string GenericInstanceMethod(IMethodSymbol method) => $"gim{UniqueIdString()}";
        public string Instruction(string opCodeName) => $"{opCodeName}{UniqueIdString()}";
        ...
        private string NameFor(MemberDeclarationSyntax node) => (Options & NamingOptions.AppendElementNameToVariables) == NamingOptions.AppendElementNameToVariables ? $"{PartsSeparator}{node.Name()}": string.Empty;
