...
public class DefinitionsFactoryBase
{
    // This method assumes that *all* Api drivers have the same enum/enum member names for modifiers.
    // If this is not the case tests may fail due to non-existing types being referenced in the generated
    // code.
    protected static string RoslynToApiDriverModifiers(INamedTypeSymbol typeSymbol, SyntaxTokenList modifiers)
    {
        var hasStaticCtor = typeSymbol.Constructors.Any(ctor => ctor is { IsStatic: true, IsImplicitlyDeclared: false });
    ...
    private static string RoslynToApiDriverModifier<TEnumAttr>(
        ...
        Func<SyntaxToken, IEnumerable<string>> mapAttribute) where TEnumAttr : Enum
    {
        ...
        var syntaxTokens = modifiers as SyntaxToken[] ?? modifiers.ToArray();
        var finalModifierList = new List<SyntaxToken>(syntaxTokens);
    ...
    private static string RoslynToApiDriverModifier(IEnumerable<SyntaxToken> modifiers, Func<SyntaxToken, string> map)
    {
